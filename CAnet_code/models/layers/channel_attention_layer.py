...
class SE_Conv_Block_3D(nn.Module):
    expansion = 4
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_out=False):
        super(SE_Conv_Block_3D, self).__init__()

        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=(3, 3, 3), padding=(1, 1, 1))
        self.gn1 = nn.GroupNorm(4, planes)
        self.relu = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv3d(planes, planes*2, kernel_size=(3, 3, 3), padding=(1, 1, 1))
        self.gn2 = nn.GroupNorm(4, planes*2)

        self.conv3 = nn.Conv3d(planes*2, planes, kernel_size=(3, 3, 3), padding=(1, 1, 1))
        self.gn3 = nn.GroupNorm(4, planes)
        ...
        self.downchannel = None
        if inplanes != planes:
            self.downchannel = nn.Sequential(nn.Conv3d(inplanes, planes * 2, kernel_size=1, stride=stride, bias=False),
                                             nn.GroupNorm(4, planes* 2),)
